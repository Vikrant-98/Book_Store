...
namespace Book_Store.Controllers
{
    ...
    public class CartController : ControllerBase
    {
        ...
        [HttpPost]
        [Authorize(Roles = "User")]
        public async Task<IActionResult> BookPlaceOdrder(PlaceOrder Info)
        {
            try
            {
                var user = HttpContext.User;
                ...
                int userID = Convert.ToInt32(user.Claims.FirstOrDefault(u => u.Type == "UserID").Value);
                var data = await _cart.BookPlaceOdrder(userID, Info);
                if (data != null)
                {
                    success = true;
        ...
        [HttpPost]
        [Authorize(Roles = "User")]
        public async Task<IActionResult> CancelPlaceOdrder(CalcelOrder Info)
        {
            try
            {
                var user = HttpContext.User;
                ...
                int userID = Convert.ToInt32(user.Claims.FirstOrDefault(u => u.Type == "UserID").Value);
                var data = await _cart.CancelPlaceOdrder(userID, Info);
                if (data != null)
                {
                    success = true;
